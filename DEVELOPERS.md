...
This repo is a Python package for DARLIN/CARLIN amplicon analysis: it aligns edited CRISPR lineage-tracing sequences to a locus-specific reference, calls a dominant allele, and converts edits into HGVS-like mutation annotations. The public entry point is `analyze_sequences()` in `darlinpy/api.py`, with top-level exports from `darlinpy/__init__.py`. Installation is standard `setuptools` via `setup.py`; there is an optional `pybind11` C++ extension for faster alignment in `darlinpy/alignment/_cas9_align.cpp`, with a Python fallback in `darlinpy/alignment/cas9_align.py`. In a local `venv`, the reliable way to enable the fast path is `pip install -e ".[fast]"` followed by `python setup.py build_ext --inplace`, then verify `darlinpy.alignment.cas9_align.HAS_CPP_IMPL` is `True`.
